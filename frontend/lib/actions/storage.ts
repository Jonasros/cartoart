...
/**
 * Upload a sculpture thumbnail to Supabase Storage
 * @returns Public URL of the uploaded sculpture thumbnail
 */
export async function uploadSculptureThumbnail(
  mapId: string,
  userId: string,
  thumbnailBlob: Blob
): Promise<string> {
  ...
  if (mapError) {
    logger.error('Map not found during sculpture thumbnail upload', { mapId, userId, error: mapError });
    throw createError.notFound('Map');
  }

  if (!map) {
    logger.error('Map not found during sculpture thumbnail upload', { mapId, userId });
    throw createError.notFound('Map');
  }

  const mapData = map as { user_id: string };
  if (mapData.user_id !== userId) {
    logger.warn('Unauthorized sculpture thumbnail upload attempt', { mapId, userId, ownerId: mapData.user_id });
    throw createError.permissionDenied('You do not have permission to upload thumbnails for this map');
  }
  ...
  try {
    ...
    logger.info('Sculpture image validated successfully', {
      mapId,
      userId,
      width: metadata.width,
      height: metadata.height,
    ...
    logger.error('Sculpture image validation failed:', { error, mapId, userId });
    throw createError.validationError('Invalid image file: file may be corrupted or not a valid image');
  }

  const fileExt = thumbnailBlob.type === 'image/webp' ? 'webp' :
                   thumbnailBlob.type === 'image/jpeg' || thumbnailBlob.type === 'image/jpg' ? 'jpg' : 'png';
  // Use a different path for sculpture thumbnails
  const fileName = `${userId}/${mapId}-sculpture.${fileExt}`;

  logger.info('Uploading sculpture thumbnail', { mapId, userId, size: thumbnailBlob.size, type: thumbnailBlob.type });
  ...
  if (error) {
    logger.error('Failed to upload sculpture thumbnail:', { error, mapId, userId, fileName });
    throw createError.storageError(`Failed to upload sculpture thumbnail: ${error.message}`);
  }
  ...
  if (!urlData?.publicUrl) {
    logger.error('Failed to get public URL for sculpture thumbnail:', { mapId, userId, fileName });
    throw createError.storageError('Failed to get public URL for sculpture thumbnail');
  }

  logger.info('Sculpture thumbnail uploaded successfully', { mapId, userId, url: urlData.publicUrl });
  return urlData.publicUrl;
}
